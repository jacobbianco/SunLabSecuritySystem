...
public class AdminView extends JPanel{
	...
	public AdminView(Firestore db) {
		...
		leftScrollPane = new JScrollPane(renderUsers(filteredUserArray, db));
	...
	private static void filterButtonPressed() {
		...
		JLabel headerLabel = new JLabel(
				"Note: You may either filter by ID, filter by date, or filter by date and time");
		...
		submitButton.addActionListener(
				e -> filterResults(id.getText().trim(), date.getText().trim(), time.getText().trim(), filterFrame));
	...
	private static void filterResults(String id, String date, String time, JFrame filterFrame) {
		...
		if (id.length() > 0 && date.length() == 0 & time.length() == 0)
			filteredUserLogArray.forEach(item -> {
				if ((Integer.toString(item.id).equals(id)))
					newArray.add(item);
			});
		else if (date.length() > 0 && time.length() == 0 & id.length() == 0)
			filteredUserLogArray.forEach(item -> {
				if (item.timestamp.toString().substring(0, 10).equals(date))
					if (!newArray.contains(item))
						newArray.add(item);
			});
		// Note: need to convert in funky way because google FB has 4 hour offset of time
		else if (time.length() > 0 && date.length() > 0 && id.length() == 0)
			filteredUserLogArray.forEach(item -> {
				...
				if (item.in_out.equals("In") && ts.getTime() > item.timestamp.toSqlTimestamp().getTime() + 14400000) {
					filteredUserLogArray.forEach(element -> {
						if (element.in_out.equals("Out") && (item.id == element.id)
								&& ts.getTime() < element.timestamp.toSqlTimestamp().getTime() + 14400000) {
							if (!newArray.contains(item))
								newArray.add(item);
	...
	public static JPanel renderUsers(ArrayList<User> userArray, Firestore db) {
		...
		for (User user : userArray) {
			...
			hasAccess.addActionListener(e -> {
				try {
					// Toggle the access
					toggleUserAccess(user.id, false, db);
			...
			noAccess.addActionListener(e -> {
				try {
					// Toggle the access
					toggleUserAccess(user.id, true, db);
					// Switch the button and update the UI
	...
	private static void toggleUserAccess(int id, boolean access, Firestore db) {
		String docId = null;
